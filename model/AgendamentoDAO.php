...
class AgendamentoDAO{
    ...
    function inserir($agendamento){
        $result = $this->con->query("INSERT INTO agendamentos (data,horario,tempo_atendimento,id_agendante,preco) VALUES ('" . $agendamento->get_data() . "', '" . $agendamento->get_horario() . "', '" . $agendamento->get_tempo_atendimento() . "', '" . $agendamento->get_id_agendante() . "', '" . $agendamento->get_preco() . "')");
    ...
    function alterar($agendamento){
        $result = $this->con->query("UPDATE agendamentos SET data = '" . $agendamento->get_data() . "', horario = '" . $agendamento->get_horario() . "', tempo_atendimento = '" . $agendamento->get_tempo_atendimento() . "', preco = '" . $agendamento->get_preco() . "' WHERE id_agendamento = " . $agendamento->get_id_agendamento());
        if($result->rowCount() > 0){
            return true;
        }else{
            return false;
